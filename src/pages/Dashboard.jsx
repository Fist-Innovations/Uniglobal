...
const KPI_DATA = [
  { title: 'Bank Upload Accuracy %',    value: '98.4%',  sub: '+2.1% vs last week', trend: 'up',   icon: CheckCircle2, accent: 'var(--primary)', light: 'rgba(37, 99, 235, 0.1)' },
  { title: 'Duplicate Detection Count', value: '12',     sub: '−15.4% detected',    trend: 'down', icon: FileWarning,  accent: '#ef4444', light: 'rgba(239, 68, 68, 0.1)' },
  { title: 'Pending Notifications',     value: '24',     sub: '6 priority alerts',  trend: 'up',   icon: Bell,         accent: '#f59e0b', light: 'rgba(245, 158, 11, 0.1)' },
  { title: 'Budget Completion Status',  value: '76%',    sub: '+4.5% this quarter', trend: 'up',   icon: BarChart3,    accent: '#a855f7', light: 'rgba(168, 85, 247, 0.1)' },
];
...
const KPICard = ({ title, value, sub, trend, icon: Icon, light, accent }) => (
  <motion.div
    whileHover={{ y: -4 }}
  ...
  >
    ...
    <div>
      <p style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '4px' }}>{title}</p>
      <h3 style={{ fontSize: '28px', fontWeight: 800, color: 'var(--text-main)', lineHeight: 1 }}>{value}</h3>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--text-main)', marginBottom: '2px' }}>Dashboard Overview</h1>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Welcome back, Admin. Here's what's happening today.</p>
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
          <button 
            ...
            style={{
              ...
              borderRadius: '10px', fontSize: '13px', fontWeight: 600, color: 'var(--text-muted)', 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px', minWidth: 0 }}>
        ...
        <div style={{ ...S.card, minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
            ...
            <span style={{ fontSize: '11px', background: 'rgba(37, 99, 235, 0.1)', color: 'var(--primary)', padding: '4px 10px', borderRadius: '20px', fontWeight: 600 }}>● Live</span>
          </div>
          ...
          <div style={{ height: '240px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', fontSize: '12px', color: 'var(--text-main)' }} cursor={{ fill: 'var(--bg-dark)', opacity: 0.4 }} />
                <Bar dataKey="accuracy" fill="var(--primary)" radius={[6, 6, 0, 0]} barSize={22} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
              ...
              {suggestions.slice(0, 3).map(s => (
                <motion.div 
                  ...
                  style={{
                    ...
                    padding: '12px 14px', background: 'var(--bg-dark)', borderRadius: '10px',
                    border: '1px solid var(--border)',
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-main)', marginBottom: '1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.desc}</p>
                      <p style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{s.gl}</p>
                    </div>
                  </div>
                  <button onClick={() => handleSuggestionAction(s.id)} style={{ padding: '4px 8px', background: 'rgba(37, 99, 235, 0.1)', border: 'none', borderRadius: '6px', fontSize: '10px', fontWeight: 600, color: 'var(--primary)', cursor: 'pointer' }}>OK</button>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
        ...
        <div style={{ ...S.card, border: '1px solid rgba(239, 68, 68, 0.2)', background: 'rgba(239, 68, 68, 0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
            <div>
              <p style={{ ...S.sectionTitle, color: '#ef4444' }}>Alerts & Exceptions</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              { text: 'Budget Mismatch: Shipping Dept', sub: 'Variance > 15% detected', color: '#ef4444' },
              { text: 'Unreconciled Bank Entries', sub: '4 transactions missing GL mapping', color: '#ef4444' },
              { text: 'Duplicate Vendor Invoice', sub: 'Invoice #INV-9901 (Potential)', color: '#ef4444' },
            ].map((alert, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px' }}>
                ...
                <div>
                  <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-main)' }}>{alert.text}</p>
                  <p style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{alert.sub}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ ...S.card }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
            {[
              { icon: CheckCircle2, color: '#10b981', bg: 'rgba(16, 185, 129, 0.1)', text: 'Bank statement uploaded successfully', sub: '2 minutes ago', badge: 'GL' },
              { icon: Bot,          color: 'var(--primary)', bg: 'rgba(37, 99, 235, 0.1)', text: 'AI processed 148 transactions',         sub: '15 minutes ago', badge: 'AI' },
              { icon: Users,        color: '#a855f7', bg: 'rgba(168, 85, 247, 0.1)', text: '3 new student notifications sent',      sub: '1 hour ago',     badge: 'TRN' },
              { icon: Activity,     color: '#ef4444', bg: 'rgba(239, 68, 68, 0.1)', text: '12 duplicate entries flagged',           sub: '2 hours ago',    badge: '!' },
            ].map((item, i, arr) => (
              <div key={i} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: '12.5px', fontWeight: 600, color: 'var(--text-main)', marginBottom: '2px' }}>{item.text}</p>
