...
const card = {background:'var(--bg-card)',borderRadius:'16px',border:'1px solid var(--border)',boxShadow:'var(--shadow-sm)',transition:'background 0.3s, border-color 0.3s'};
const th = {padding:'11px 16px',fontSize:'10.5px',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'.7px',background:'var(--bg-dark)',textAlign:'left'};
const td = {padding:'13px 16px',fontSize:'13px',borderBottom:'1px solid var(--border)'};
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:'16px',height:'calc(100vh - 120px)'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
          <h1 style={{fontSize:'22px',fontWeight:800,color:'var(--text-main)',letterSpacing:'-.5px'}}>GL AI Bot</h1>
          <p style={{fontSize:'13px',color:'var(--text-muted)',marginTop:'2px'}}>Natural language queries, anomaly detection & ledger insights</p>
        </div>
      ...
      <div style={{flex:1,display:'flex',gap:'16px',minHeight:0}}>
        ...
        <div style={{width:'240px',flexShrink:0,...card,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{padding:'14px 12px',borderBottom:'1px solid var(--border)'}}>
            <button onClick={startNewChat} style={{width:'100%',padding:'10px',background:'var(--primary)',border:'none',borderRadius:'10px',fontSize:'13px',fontWeight:700,color:'#fff',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'7px',boxShadow:'0 4px 12px rgba(29,78,216,.25)'}}>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'8px'}}>
            {[{label:'Today',items:sessions.filter((_,i)=>i<5)},{label:'Earlier',items:sessions.filter((_,i)=>i>=5)}].map(group=>(
              <div key={group.label}>
                <p style={{fontSize:'10px',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'1px',padding:'10px 8px 6px'}}>{group.label}</p>
                {group.items.map(s=>(
                  <button key={s.id} onClick={()=>setActiveSession(s.id)}
                    style={{width:'100%',padding:'10px 10px',borderRadius:'10px',border:'none',cursor:'pointer',background:activeSession===s.id?'rgba(37, 99, 235, 0.1)':'transparent',textAlign:'left',marginBottom:'2px',transition:'background .15s'}}
                    onMouseEnter={e=>{if(activeSession!==s.id)e.currentTarget.style.background='var(--bg-dark)';}}
                    onMouseLeave={e=>{if(activeSession!==s.id)e.currentTarget.style.background='transparent';}}>
                    <p style={{fontSize:'12.5px',fontWeight:600,color:activeSession===s.id?'var(--primary)':'var(--text-main)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',marginBottom:'2px'}}>{s.title}</p>
                    <p style={{fontSize:'11px',color:'var(--text-muted)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s.preview}</p>
                    <p style={{fontSize:'10px',color:'var(--text-muted)',marginTop:'3px'}}>{s.time}</p>
                  </button>
                ))}
              </div>
            ))}
          ...
          <div style={{ padding: '16px 12px', borderTop: '1px solid var(--border)', background: 'var(--bg-dark)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              <div style={{ width: '24px', height: '24px', borderRadius: '6px', background: 'rgba(37, 99, 235, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Sparkles size={14} color="var(--primary)" />
              </div>
              <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Model Status</span>
            </div>
            <div style={{ padding: '8px 10px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px' }}>
              <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-main)' }}>GL-Finance v4.2</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#22c55e' }} />
                <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>System Optimal</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {tab==='chat' && (
            <div style={{flex:1,display:'flex',flexDirection:'column',...card,overflow:'hidden'}}>
              ...
              <div style={{padding:'16px 20px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:'12px',flexShrink:0}}>
                ...
                <div>
                  <p style={{fontSize:'14px',fontWeight:700,color:'var(--text-main)'}}>UNIGLOBAL Finance GPT</p>
              ...
              <div style={{flex:1,overflowY:'auto',padding:'20px',display:'flex',flexDirection:'column',gap:'16px'}}>
                {msgs.map((m,i)=>(
                  <motion.div key={i} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i<DUMMY_HISTORY.length?0:0}}
                    ...
                    <div style={{maxWidth:'68%',padding:'12px 15px',borderRadius:m.role==='user'?'16px 4px 16px 16px':'4px 16px 16px 16px',background:m.role==='user'?'var(--primary)':'var(--bg-dark)',color:m.role==='user'?'#fff':'var(--text-main)',fontSize:'13.5px',lineHeight:'1.65',border:m.role==='assistant'?'1px solid var(--border)':'none',fontWeight:450}}>
                      {m.text}
                      {m.hasResult && (
                ...
                {loading && (
                  <div style={{display:'flex',gap:'10px',alignItems:'flex-start'}}>
                    ...
                    <div style={{padding:'12px 15px',background:'var(--bg-dark)',borderRadius:'4px 16px 16px 16px',border:'1px solid var(--border)',display:'flex',gap:'5px',alignItems:'center'}}>
                      {[0,1,2].map(d=><span key={d} style={{width:'7px',height:'7px',borderRadius:'50%',background:'var(--primary)',animation:`dot 1s ${d*.2}s infinite`,display:'block'}}/>)}
                    </div>
                  </div>
                )}
                <div ref={endRef}/>
              ...
              <div style={{padding:'12px 20px',borderTop:'1px solid var(--border)',background:'var(--bg-dark)',flexShrink:0}}>
                <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'10px'}}>
                  <Zap size={13} color="var(--primary)"/>
                  <span style={{fontSize:'11px',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'.8px'}}>Quick Queries</span>
                </div>
                <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'12px'}}>
                  {QUICK.map(q=>(
                    <button key={q} onClick={()=>send(q)}
                      style={{padding:'6px 12px',background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:'20px',fontSize:'11.5px',fontWeight:600,color:'var(--text-muted)',cursor:'pointer',transition:'all .15s'}}
                      onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--primary)';e.currentTarget.style.color='var(--primary)';e.currentTarget.style.background='rgba(37, 99, 235, 0.1)';}}
                ...
                <div style={{display:'flex',gap:'10px',alignItems:'center'}}>
                  <div style={{position:'relative',flex:1}}>
                    ...
                    <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()}
                      ...
                      style={{width:'100%',padding:'12px 14px 12px 42px',border:'1.5px solid var(--border)',borderRadius:'12px',fontSize:'13px',outline:'none',background:'var(--bg-card)',color:'var(--text-main)',boxSizing:'border-box',transition:'border-color .2s'}}
            ...
            <div style={{flex:1,display:'flex',flexDirection:'column',gap:'16px',overflowY:'auto'}}>
              {!result ? (
                <div style={{...card,padding:'60px',textAlign:'center'}}>
                  <BarChart3 size={40} color="var(--border)" style={{margin:'0 auto 14px',display:'block'}}/>
                  <p style={{fontSize:'16px',fontWeight:700,color:'var(--text-main)'}}>No query results yet</p>
                  <p style={{fontSize:'13px',color:'var(--text-muted)',marginTop:'4px'}}>Ask something in the GL Chat tab first</p>
                  <button onClick={()=>setTab('chat')} style={{marginTop:'16px',padding:'10px 22px',background:'var(--primary)',border:'none',borderRadius:'10px',fontSize:'13px',fontWeight:700,color:'#fff',cursor:'pointer'}}>Go to Chat</button>
                </div>
              ):(
                <>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'14px'}}>
                    {result.data.map(d=>(
                      <div key={d.label} style={{...card,padding:'18px'}}>
                        <p style={{fontSize:'10px',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'.6px',marginBottom:'8px'}}>{d.label}</p>
                        <p style={{fontSize:'24px',fontWeight:800,color:'var(--text-main)',letterSpacing:'-.5px'}}>{d.value}</p>
                  ...
                  <div style={{...card,overflow:'hidden'}}>
                    <div style={{padding:'16px 20px',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      <div><p style={{fontSize:'15px',fontWeight:700,color:'var(--text-main)'}}>Ledger Summary</p><p style={{fontSize:'12px',color:'var(--text-muted)'}}>Click any row to expand transactions</p></div>
                      <div style={{display:'flex',gap:'8px'}}>
                        <button onClick={()=>dl('gl_report.pdf','GL Report PDF')} style={{padding:'7px 13px',background:'var(--bg-dark)',border:'1px solid var(--border)',borderRadius:'8px',fontSize:'12px',fontWeight:600,color:'var(--text-muted)',cursor:'pointer',display:'flex',alignItems:'center',gap:'5px'}}><Download size={13}/>PDF</button>
                        <button onClick={()=>dl('gl_data.csv',result.table.map(r=>`${r.account},${r.debit},${r.credit},${r.bal}`).join('\n'))} style={{padding:'7px 13px',background:'var(--bg-dark)',border:'1px solid var(--border)',borderRadius:'8px',fontSize:'12px',fontWeight:600,color:'var(--text-muted)',cursor:'pointer',display:'flex',alignItems:'center',gap:'5px'}}><Download size={13}/>Excel</button>
                      </div>
                    </div>
                    <table style={{width:'100%',borderCollapse:'collapse'}}>
                      ...
                      <tbody>
                        {result.table.map((row,i)=>(
                          <>
                            <tr key={i} onClick={()=>setExpanded(p=>({...p,[i]:!p[i]}))} style={{background:i%2===0?'var(--bg-card)':'var(--bg-dark)',cursor:'pointer'}}>
                              <td style={{...td,fontWeight:600,color:'var(--text-main)'}}>{row.account}</td>
                              ...
                              <td style={td}><span style={{padding:'3px 9px',borderRadius:'20px',fontSize:'11px',fontWeight:700,background:row.status==='Revenue'?'rgba(22, 163, 74, 0.1)':'rgba(37, 99, 235, 0.1)',color:row.status==='Revenue'?'#16a34a':'var(--primary)'}}>{row.status}</span></td>
                              <td style={td}>{expanded[i]?<ChevronDown size={15} color="var(--text-muted)"/>:<ChevronRight size={15} color="var(--text-muted)"/>}</td>
                            </tr>
                            {expanded[i] && <tr key={`e${i}`}><td colSpan={6} style={{background:'rgba(37, 99, 235, 0.05)',padding:'0'}}><div style={{padding:'14px 20px',borderLeft:'3px solid var(--primary)'}}><p style={{fontSize:'11px',fontWeight:700,color:'var(--primary)',textTransform:'uppercase',marginBottom:'8px'}}>Transaction Breakdown</p>{[1,2,3].map(n=><div key={n} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid var(--border)',fontSize:'12.5px'}}><span style={{color:'var(--text-main)'}}>2026-04-{n*5+10} · Entry #{n}</span><span style={{fontWeight:700,color:'var(--text-main)'}}>-${(n*420+150).toFixed(2)}</span></div>)}</div></td></tr>}
                          </>
                        ))}
                      </tbody>
                    </table>
          ...
          {tab==='anomaly' && (
            <div style={{flex:1,display:'flex',flexDirection:'column',gap:'16px',overflowY:'auto'}}>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'14px'}}>
                {[
                  ...
                  <div key={s.label} style={{...card,padding:'20px',display:'flex',alignItems:'center',gap:'16px'}}>
                    ...
                    <div>
                      <p style={{fontSize:'26px',fontWeight:800,color:'var(--text-main)',letterSpacing:'-.5px'}}>{s.val}</p>
                      <p style={{fontSize:'12px',color:'var(--text-muted)',fontWeight:600,marginTop:'2px'}}>{s.label}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <p style={{fontSize:'15px',fontWeight:700,color:'var(--text-main)'}}>Detected Anomalies</p>
                </div>
                <AnimatePresence>
                  {anomalies.map((a,i)=>(
                    <motion.div key={a.id} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} exit={{opacity:0,height:0,overflow:'hidden'}} transition={{delay:i*.05}}
                      ...
                      <div style={{flex:1}}>
                        <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'3px',flexWrap:'wrap'}}>
                          <span style={{fontWeight:700,fontSize:'13.5px',color:'var(--text-main)'}}>{a.desc}</span>
                          ...
                          <span style={{padding:'2px 8px',borderRadius:'20px',fontSize:'10px',fontWeight:700,background:'var(--bg-dark)',color:'var(--text-muted)'}}>{a.type}</span>
                        </div>
                        <p style={{fontSize:'12px',color:'var(--text-muted)'}}>Amount: <strong>{a.amt}</strong> · {a.date} · Confidence: <strong style={{color:SEV[a.sev]}}>{a.conf}%</strong></p>
                      </div>
                      <div style={{display:'flex',gap:'8px',flexShrink:0}}>
                        <button onClick={()=>setAnomalies(p=>p.filter(x=>x.id!==a.id))} style={{padding:'7px 13px',background:'var(--bg-dark)',border:'1px solid var(--border)',borderRadius:'8px',fontSize:'12px',fontWeight:600,color:'var(--text-muted)',cursor:'pointer',display:'flex',alignItems:'center',gap:'5px'}}><X size={13}/>Dismiss</button>
                        <button onClick={()=>alert(`Reviewing: ${a.desc}`)} style={{padding:'7px 13px',background:'rgba(37, 99, 235, 0.1)',border:'1px solid var(--border)',borderRadius:'8px',fontSize:'12px',fontWeight:600,color:'var(--primary)',cursor:'pointer'}}>Review</button>
                      </div>
                    </motion.div>
                  ))}
                </AnimatePresence>
                {anomalies.length===0 && <div style={{...card,padding:'48px',textAlign:'center'}}><p style={{color:'var(--text-muted)',fontWeight:600,fontSize:'14px'}}>All anomalies have been reviewed</p></div>}
              </div>
            </div>
          )}
